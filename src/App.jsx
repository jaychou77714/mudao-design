...
function ProjectCard({ p, onClick }) {
  const { totalPrice, collectRate, netProfitPct } = calcProfit(p);
  ...
  return (
    <div onClick={onClick} className="glass-card" style={{ borderLeft: `3px solid ${STAGE_COLORS[p.stage]}`, borderRadius: 14, padding: "14px 16px", marginBottom: 10, cursor: "pointer", transition: "transform 0.15s, box-shadow 0.15s" }}
      ...
      {p.stage === "提案中" && p.consultDate && (() => {
        ...
        if (p.proposalDate) return <div style={{ fontSize: 10, color: "#50c878", marginTop: 3 }}>✅ 提案時間已訂</div>;
        if (remaining <= 0) return <div style={{ fontSize: 10, color: "#e05b5b", marginTop: 3 }}>🚨 已超過15工作天！</div>;
        if (remaining <= 3) return <div style={{ fontSize: 10, color: "#f06450", marginTop: 3 }}>⚠️ 剩 {remaining} 天！快訂提案時間</div>;
        if (remaining <= 7) return <div style={{ fontSize: 10, color: "#f0a850", marginTop: 3 }}>📋 剩 {remaining} 工作天</div>;
        return null;
      })()}
    </div>
  );
...
export default function App() {
  ...
  function reviveFromTrash(trashItem) {
    ...
    <div style={{ minHeight: "100vh", background: "rgba(255,255,255,0.04)", color: "#e8e4dc", fontFamily: "'Noto Sans TC',sans-serif", maxWidth: 480, margin: "0 auto", display: "flex", flexDirection: "column", position: "relative" }}>
          ...
          <div style={{ display: "flex", alignItems: "baseline", gap: 8 }}>
            ...
            {editingName ? (
              <input autoFocus value={userName} onChange={e => setUserName(e.target.value)}
                ...
                placeholder="輸入名稱"
                style={{ background: "rgba(255,255,255,0.08)", border: "1px solid rgba(255,255,255,0.2)", borderRadius: 8, padding: "3px 10px", color: "#f0ead8", fontSize: 13, outline: "none", marginLeft: 8, width: 100, fontFamily: "'Noto Sans TC', sans-serif" }} />
            ) : (
              <span onClick={() => setEditingName(true)} style={{ fontSize: 12, color: userName ? "#c8b89a" : "#444", marginLeft: 8, cursor: "pointer", borderBottom: "1px dashed rgba(200,184,154,0.3)", paddingBottom: 1 }}>
                {userName || "點此輸入名稱"}
              </span>
            )}
          </div>
          ...
          <div className="fade-up">
            <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 10, marginBottom: 20 }}>
              {STAGES.map(s => {
                ...
                // 找最久未更新的案件（用 id 最小的當作最舊）
                const stageList = projects.filter(p => p.stage === s);
                const oldest = stageList.length > 0 ? stageList.reduce((a, b) => (a.id < b.id ? a : b)) : null;
                ...
                return (
                  <div key={s} onClick={() => setTab(s)} className="glass-stat" style={{ borderRadius: 16, padding: "14px 16px", position: "relative", overflow: "hidden", minHeight: 100 }}>
                    {svgBgs[s]}
                    <div style={{ position: "relative", zIndex: 1 }}>
                      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", marginBottom: 6 }}>
                        <div style={{ fontSize: 30, fontWeight: 700, color: STAGE_COLORS[s], lineHeight: 1 }}>{count}</div>
                      ...
                      {oldest ? (
                        <div style={{ fontSize: 11, color: STAGE_COLORS[s], fontWeight: 600, marginTop: 8, whiteSpace: "nowrap", overflow: "hidden", textOverflow: "ellipsis" }}>
                          {oldest.name}
                        </div>
                      ) : (
                        <div style={{ fontSize: 11, color: "#333", marginTop: 8 }}>尚無案件</div>
                      )}
                    </div>
                  </div>
                );
              ...
              {/* 垃圾場卡片 */}
              <div onClick={() => setTab("垃圾場")} className="glass-stat" style={{ borderRadius: 16, padding: "14px 16px", position: "relative", overflow: "hidden", minHeight: 100 }}>
                ...
                <div style={{ position: "relative", zIndex: 1 }}>
                  <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", marginBottom: 6 }}>
                    <div style={{ fontSize: 30, fontWeight: 700, color: "#e05b5b", lineHeight: 1 }}>{trashProjects.length}</div>
                  ...
                  {trashProjects.length > 0 ? (
                    <div style={{ fontSize: 11, color: "#e05b5b", fontWeight: 600, marginTop: 8, whiteSpace: "nowrap", overflow: "hidden", textOverflow: "ellipsis" }}>
                      {trashProjects[trashProjects.length - 1]?.name}
                    </div>
                  ) : (
                    <div style={{ fontSize: 11, color: "#333", marginTop: 8 }}>保持清空！</div>
                  )}
                </div>
              </div>
            </div>
      ...
      {modal === "detail" && detailProject && (() => {
        ...
        const sectionTabs = ["info",
          ...
          ...(p.stage !== "提案中" && p.stage !== "已簽約" ? ["報價單"] : []),
          ...(p.stage === "施工中" || p.stage === "完工" ? ["追加減", "額外追加", "驗收缺失", "工班請款", "吸收成本"] : []),
        ];
          ...
          <Modal onClose={() => setModal(null)}>
            ...
            {detailSection === "info" && (
              <div>
                ...
                <DetailRow label="諮詢/丈量" value={p.consultDate} />
                {p.indoorArea && <DetailRow label="室內坪數" value={`${p.indoorArea} 坪`} />}
                ...
                {p.stage === "提案中" && (
                  <>
                    {p.layoutDate && <DetailRow label="配置圖完成" value={`${p.layoutDate}${p.consultDate ? `（${calcWorkDays(p.consultDate, p.layoutDate)} 工作天）` : ""}`} />}
                    {p.render3dDate && <DetailRow label="3D圖完成" value={`${p.render3dDate}${p.consultDate ? `（${calcWorkDays(p.consultDate, p.render3dDate)} 工作天）` : ""}`} />}
                    <DetailRow label="報價單完成" value={p.quoteReady} />
            ...
            {detailSection === "報價單" && (
              <div>
                ...
                {WORK_ITEMS.map(item => {
                  ...
                  if (!q.price && !q.cost && !co) return null;
                  return (
                    <div key={item} style={{ display: "grid", gridTemplateColumns: "1fr 80px 80px", gap: 4, padding: "7px 0", borderBottom: "1px solid rgba(255,255,255,0.07)" }}>
                      <div>
                        ...
                        {co ? <div style={{ fontSize: 10, color: num(co) >= 0 ? "#5b8af0" : "#e05b5b" }}>追加減 {num(co) >= 0 ? "+" : ""}{fmtMoney(num(co))}</div> : null}
                      </div>
                ...
                {(p.extraItems || []).length > 0 && (
                  <>
                    <div style={{ fontSize: 10, color: "#5b8af0", margin: "10px 0 6px", letterSpacing: 1 }}>── 額外追加 ──</div>
                    {(p.extraItems || []).map(e => (
                      <div key={e.id} style={{ display: "grid", gridTemplateColumns: "1fr 80px 80px", gap: 4, padding: "7px 0", borderBottom: "1px solid rgba(255,255,255,0.07)", opacity: e.collected ? 1 : 0.5 }}>
                        <div>
                ...
                <div style={{ marginTop: 12, padding: "12px", background: "rgba(255,255,255,0.05)", borderRadius: 10 }}>
                  <div style={{ display: "flex", justifyContent: "space-between", marginBottom: 4 }}>
                    <span style={{ fontSize: 12, color: "#555" }}>原始報價</span>
                    <span style={{ fontSize: 13, color: "#888" }}>$ {fmtMoney(basePrice)}</span>
                  </div>
                  {totalChangeOrders !== 0 && (
                    <div style={{ display: "flex", justifyContent: "space-between", marginBottom: 4 }}>
                      <span style={{ fontSize: 12, color: "#555" }}>追加減合計</span>
                      <span style={{ fontSize: 13, color: totalChangeOrders >= 0 ? "#5b8af0" : "#e05b5b" }}>{totalChangeOrders >= 0 ? "+" : ""}$ {fmtMoney(totalChangeOrders)}</span>
                    </div>
                  )}
                  <div style={{ display: "flex", justifyContent: "space-between", marginBottom: 6 }}>
                  ...
                  <div style={{ display: "flex", justifyContent: "space-between", marginBottom: 6 }}>
                    <span style={{ fontSize: 12, color: "#555" }}>成本總計</span>
                    <span style={{ fontSize: 13, color: "#f0a850" }}>－ $ {fmtMoney(totalCost)}</span>
                  </div>
                  {extraRevenue > 0 && (
                    <div style={{ display: "flex", justifyContent: "space-between", marginBottom: 6 }}>
                  ...
                  <div style={{ display: "flex", justifyContent: "space-between" }}>
                    <span style={{ fontSize: 13, color: "#ccc", fontWeight: 600 }}>粗估毛利</span>
                    <span style={{ fontSize: 15, color: "#5b8af0", fontWeight: 700 }}>$ {fmtMoney(grossProfit)}</span>
                  </div>
                </div>
              </div>
            )}
        ...
        <Modal onClose={() => setModal(null)} title={form.id ? "編輯案件" : `新增${form.stage}案件`}>
          <div style={{ display: "flex", gap: 6, marginBottom: 16, overflowX: "auto" }}>
            {["basic",
              ...
              ...(form.stage === "施工中" || form.stage === "完工" ? ["施工", "報價", "追加減", "額外追加", "吸收成本", "工班請款"] : [])
            ].map(s => (
                  ...
                  return (
                    <div key={pay.id} style={{ background: "rgba(255,255,255,0.04)", border: "1px solid rgba(255,255,255,0.08)", borderRadius: 12, padding: "12px", marginBottom: 10 }}>
                      ...
                      <Field label="付款日期">
                        ...
                        {payRating && <div style={{ fontSize: 11, color: "#5b8af0", marginTop: 4 }}>{payRating} 簽約後 {daysFromContract} 天收款</div>}
                      </Field>
                    </div>
                  );
                })}
