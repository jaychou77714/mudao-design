...
const EMPTY_PROJECT = {
  ...
  indoorArea: "", // 室內坪數
  meetingDates: [], // 開會時間 [{id, date}]
  layoutDate: "", // 配置圖完成日期
  render3dDate: "", // 3D圖完成日期
  proposalDate: "", // 提案時間
  quoteReady: "否", // 報價單是否完成
  signedDate: "", // 案子簽約日期
...
export default function App() {
  ...
  function openNew(stage = "提案中") {
    setForm({ ...EMPTY_PROJECT, id: null, stage, quote: DEFAULT_QUOTE(), changeOrders: DEFAULT_BILLING(), extraItems: [], absorbedCosts: [], billing: DEFAULT_BILLING(), defects: [], payments: [], meetingDates: [] });
    setFormTab("basic");
    setModal("new");
  }

  function openEdit(p) {
    setForm({ ...p, quote: p.quote || DEFAULT_QUOTE(), changeOrders: p.changeOrders || DEFAULT_BILLING(), extraItems: p.extraItems || [], absorbedCosts: p.absorbedCosts || [], billing: p.billing || DEFAULT_BILLING(), defects: p.defects || [], payments: p.payments || [], meetingDates: p.meetingDates || [] });
    setFormTab("basic");
    setModal("edit");
  }
          ...
          <Modal onClose={() => setModal(null)}>
            ...
            <div style={{ display: "flex", gap: 6, marginBottom: 20, overflowX: "auto", borderBottom: "1px solid rgba(255,255,255,0.07)", paddingBottom: 16 }}>
              {sectionTabs.map(s => (
                ...
                }}>{s === "info" ? "📋 基本" : s === "提案後狀態" ? "📝 提案後" : s === "報價單" ? "💰 報價" : s === "追加減" ? "🔄 追加減" : s === "額外追加" ? "➕ 額外" : s === "驗收缺失" ? "✅ 驗收" : s === "工班請款" ? "🧾 請款" : "💸 吸收"}</button>
              ))}
            </div>
