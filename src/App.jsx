...
const EMPTY_PROJECT = {
  ...
  consultDate: "", budget: "", designer: "", assistant: "", siteManager: "", notes: "",
...
export default function App() {
  ...
  function openNew(stage = "提案中") {
    setForm({ ...EMPTY_PROJECT, id: null, stage, quote: DEFAULT_QUOTE(), changeOrders: DEFAULT_BILLING(), extraItems: [], absorbedCosts: [], billing: DEFAULT_BILLING(), defects: [], payments: [] });
    setFormTab("basic");
    setModal("new");
  }

  function openEdit(p) {
    setForm({ ...p, quote: p.quote || DEFAULT_QUOTE(), changeOrders: p.changeOrders || DEFAULT_BILLING(), extraItems: p.extraItems || [], absorbedCosts: p.absorbedCosts || [], billing: p.billing || DEFAULT_BILLING(), defects: p.defects || [], payments: p.payments || [] });
    setFormTab("basic");
    setModal("edit");
  }
          ...
          <Modal onClose={() => setModal(null)}>
            ...
            <div style={{ display: "flex", gap: 6, marginBottom: 20, overflowX: "auto", borderBottom: "1px solid rgba(255,255,255,0.07)", paddingBottom: 16 }}>
              {sectionTabs.map(s => (
                ...
                }}>{s === "info" ? "📋 基本" : s === "報價單" ? "💰 報價" : s === "追加減" ? "🔄 追加減" : s === "額外追加" ? "➕ 額外" : s === "驗收缺失" ? "✅ 驗收" : s === "工班請款" ? "🧾 請款" : "💸 吸收"}</button>
              ))}
            </div>
            ...
            {detailSection === "info" && (
              <div>
                ...
                {p.siteManager && <DetailRow label="現場工務" value={p.siteManager} />}
      ...
      {(modal === "new" || modal === "edit") && (
        <Modal onClose={() => setModal(null)} title={form.id ? "編輯案件" : `新增${form.stage}案件`}>
          <div style={{ display: "flex", gap: 6, marginBottom: 16, overflowX: "auto" }}>
            {["basic", ...(form.stage === "施工中" || form.stage === "完工" ? ["施工", "報價", "追加減", "額外追加", "吸收成本", "工班請款"] : [])].map(s => (
          ...
          {formTab === "basic" && (
            <>
              ...
              <Field label="現場工務"><TextInput value={form.siteManager || ""} onChange={f("siteManager")} placeholder="工務姓名" /></Field>
