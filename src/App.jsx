...
const EMPTY_PROJECT = {
  ...
  quote: DEFAULT_QUOTE(),
  changeOrders: DEFAULT_BILLING(), // 追加減項目（同工項）
  extraItems: [], // 額外追加項目 [{id, name, price, cost, collected}]
  defects: [],
...
function calcProfit(p) {
  ...
  const totalPrice = basePrice + totalChangeOrders; // 最終客戶報價
  ...
  // 粗估毛利（施工中）= 報價 - 成本 + 已收額外追加毛利
  const grossProfit = totalPrice - totalCost + extraProfit;
  const grossProfitPct = totalPrice > 0 ? (grossProfit / (totalPrice + extraRevenue)) * 100 : 0;

  // 淨利（完工）= 報價 - 工班請款 + 已收額外追加毛利
  const netProfit = totalPrice - totalBilling + extraProfit;
...
export default function App() {
  ...
  function openNew(stage = "提案中") {
    setForm({ ...EMPTY_PROJECT, id: null, stage, quote: DEFAULT_QUOTE(), changeOrders: DEFAULT_BILLING(), extraItems: [], billing: DEFAULT_BILLING(), defects: [], payments: [] });
    setFormTab("basic");
    setModal("new");
  }

  function openEdit(p) {
    setForm({ ...p, quote: p.quote || DEFAULT_QUOTE(), changeOrders: p.changeOrders || DEFAULT_BILLING(), extraItems: p.extraItems || [], billing: p.billing || DEFAULT_BILLING(), defects: p.defects || [], payments: p.payments || [] });
    setFormTab("basic");
    setModal("edit");
  }
  ...
  return (
    <div style={{ minHeight: "100vh", background: "rgba(255,255,255,0.04)", color: "#e8e4dc", fontFamily: "'Noto Sans TC',sans-serif", maxWidth: 480, margin: "0 auto", display: "flex", flexDirection: "column", position: "relative" }}>
      ...
      {modal === "detail" && detailProject && (() => {
        ...
        const { basePrice, totalPrice, totalCost, totalBilling, totalChangeOrders, extraRevenue, extraCost, extraProfit, grossProfit, grossProfitPct, netProfit, netProfitPct, totalPayments, collectRate, profitPct, finalProfit } = calcProfit(p);
          ...
          <Modal onClose={() => setModal(null)}>
            ...
            <div style={{ display: "flex", gap: 6, marginBottom: 20, overflowX: "auto", borderBottom: "1px solid rgba(255,255,255,0.07)", paddingBottom: 16 }}>
              {sectionTabs.map(s => (
                ...
                }}>{s === "info" ? "📋 基本資料" : s === "報價單" ? "💰 報價單" : s === "驗收缺失" ? "✅ 驗收缺失" : "🧾 工班請款"}</button>
              ))}
            </div>
      ...
      {(modal === "new" || modal === "edit") && (
        <Modal onClose={() => setModal(null)} title={form.id ? "編輯案件" : `新增${form.stage}案件`}>
          <div style={{ display: "flex", gap: 6, marginBottom: 16, overflowX: "auto" }}>
            {["basic", ...(form.stage === "施工中" || form.stage === "完工" ? ["施工", "報價", "追加減", "額外追加", "工班請款"] : [])].map(s => (
