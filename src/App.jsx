...
function Modal({ children, onClose, title }) {
  return (
    <div style={{ position: "fixed", inset: 0, background: "rgba(0,0,0,0.75)", zIndex: 100, display: "flex", alignItems: "flex-end", backdropFilter: "blur(4px)", WebkitBackdropFilter: "blur(4px)" }}
      onClick={e => e.target === e.currentTarget && onClose()}>
      <div className="glass-modal" style={{ borderRadius: "22px 22px 0 0", width: "100%", maxHeight: "92vh", overflowY: "auto", padding: "16px 20px 48px", border: "1px solid rgba(255,255,255,0.1)", borderBottom: "none" }}>
        <div style={{ width: 36, height: 4, background: "rgba(255,255,255,0.15)", borderRadius: 2, margin: "0 auto 14px" }} />
        {title && <div style={{ fontSize: 16, fontWeight: 700, color: "#f0ead8", marginBottom: 14 }}>{title}</div>}
        {children}
      </div>
    </div>
  );
    ...
    <div style={{ minHeight: "100vh", background: "rgba(255,255,255,0.04)", color: "#e8e4dc", fontFamily: "'Noto Sans TC',sans-serif", maxWidth: 480, margin: "0 auto", display: "flex", flexDirection: "column", position: "relative" }}>
      ...
      <div className="glass-header" style={{ padding: "48px 20px 16px", position: "relative", zIndex: 10 }}>
        ...
        <div style={{ display: "flex", alignItems: "baseline", gap: 8, marginBottom: 16 }}>
          ...
          <span style={{ fontSize: 12, color: "#444", marginLeft: 4 }}>個人案件追蹤</span>
        </div>
      ...
      <div className="glass-nav" style={{ display: "flex", overflowX: "auto", position: "relative", zIndex: 10 }}>
        {allTabs.map(t => {
          const label = t === "home" ? "總覽" : t === "備忘錄" ? "備忘" : t;
          const color = t === "home" ? "#e8e4dc" : t === "備忘錄" ? "#c084f5" : STAGE_COLORS[t];
          return (
            <button key={t} className="nav-tab" onClick={() => setTab(t)} style={{
              flex: 1, padding: "11px 4px", fontSize: 11, fontWeight: tab === t ? 700 : 400,
              color: tab === t ? color : "#3a3a3e", background: "none", border: "none",
              borderBottom: tab === t ? `2px solid ${color}` : "2px solid transparent", whiteSpace: "nowrap",
            }}>{label}</button>
          );
        })}
      </div>
        ...
        {tab === "home" && (
          <div className="fade-up">
            <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 10, marginBottom: 20 }}>
              {STAGES.map(s => (
                <div key={s} onClick={() => setTab(s)} className="glass-stat" style={{ borderRadius: 14, padding: "16px" }}>
                  <div style={{ fontSize: 24, fontWeight: 700, color: STAGE_COLORS[s], marginBottom: 4 }}>{stageProjects(s).length}</div>
                  <div style={{ fontSize: 12, color: "#666" }}>{s}</div>
                </div>
              ))}
              <div onClick={() => setTab("備忘錄")} className="glass-stat" style={{ borderRadius: 14, padding: "16px" }}>
                <div style={{ fontSize: 24, fontWeight: 700, color: "#c084f5", marginBottom: 4 }}>{memos.length}</div>
        ...
        return (
          <Modal onClose={() => setModal(null)}>
            <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", marginBottom: 14 }}>
              <div>
                <StageTag stage={p.stage} />
                <h2 style={{ fontSize: 18, fontWeight: 700, color: "#f0ead8", marginTop: 8, lineHeight: 1.3 }}>{p.name}</h2>
                <div style={{ fontSize: 12, color: "#555", marginTop: 3 }}>{p.client} · {p.address}</div>
              </div>
              <div style={{ display: "flex", gap: 6 }}>
                {p.stage !== "完工" && (
                  <button className="btn" onClick={() => openEdit(p)} style={{ background: "rgba(255,255,255,0.08)", border: "none", borderRadius: 8, padding: "6px 12px", color: "#aaa", fontSize: 12 }}>編輯</button>
                )}
                <button className="btn" onClick={() => deleteProject(p.id)} style={{ background: "rgba(224,91,91,0.15)", border: "none", borderRadius: 8, padding: "6px 10px", color: "#e05b5b", fontSize: 12 }}>刪除</button>
              </div>
            </div>
            ...
            {nextStage && (
              <button className="btn" onClick={() => advanceStage(p.id)} style={{
                ...
                background: `${STAGE_COLORS[nextStage]}18`, border: `1px solid ${STAGE_COLORS[nextStage]}44`,
                borderRadius: 12, padding: "12px", color: STAGE_COLORS[nextStage], fontSize: 14, fontWeight: 600,
              }}>轉移至「{nextStage}」→</button>
            )}
          </Modal>
        );
      })()}
